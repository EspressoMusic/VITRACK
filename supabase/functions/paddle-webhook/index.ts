...
//
// Receives Paddle Billing webhooks and records verified subscription status in
// `paddle_subscriptions`. This is the only writer of that table (besides
// link-paddle-subscription) — the app must never trust a client-reported
// "I paid" event for anything beyond an optimistic, revocable local unlock.
//
// Auth is the Paddle-Signature HMAC, not a Supabase JWT — Paddle has no Supabase
// session, so this function is deployed with --no-verify-jwt and authenticates
// the request itself.
...
Deno.serve(async (req) => {
  ...
  if (typeof event.event_type === 'string' && event.event_type.startsWith('subscription.') && event.data) {
    ...
    if (error) {
      console.error('Failed to record Paddle subscription:', error)
