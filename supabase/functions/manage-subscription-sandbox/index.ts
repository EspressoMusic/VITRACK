// Deploy with: supabase functions deploy manage-subscription-sandbox
//
// Sandbox-only twin of manage-subscription — same logic, but hardcoded to Paddle's sandbox
// API and reading the PADDLE_SANDBOX_* secrets, so testing never touches the live
...
const PLAN_PRICE: Record<'monthly' | 'yearly', string | undefined> = {
  monthly: Deno.env.get('PADDLE_SANDBOX_PRICE_MONTHLY'),
  yearly: Deno.env.get('PADDLE_SANDBOX_PRICE_YEARLY'),
}
...
Deno.serve(async (req) => {
  ...
  if (!paddleApiKey) return jsonResponse({ error: 'Sandbox subscription management is not configured.' }, 500)
  ...
  try {
    ...
    console.error(`manage-subscription-sandbox (${action}) failed:`, err)
